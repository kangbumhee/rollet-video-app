...
export const advanceGame = onCall({ region: "asia-northeast3" }, async (request) => {
  ...
  switch (action) {
    ...
    case "create_round": {
      ...
      // 생존자 목록
      const participants = session.participants || [];
      const alivePlayers = participants.filter((p: { eliminated?: boolean; uid: string }) => !p.eliminated).map((p: { uid: string }) => p.uid);

      // 셔플
      const shuffled = [...alivePlayers].sort(() => Math.random() - 0.5);
      const matchups: Array<{ matchId: string; player1Id: string; player2Id: string; status: string }> = [];
