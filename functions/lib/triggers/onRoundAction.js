...
exports.onRoundActionSubmit = void 0;
...
exports.onRoundActionSubmit = (0, database_1.onValueWritten)({
    ...
    try {
        ...
        if (current.phase !== 'playing')
            return;
        if (`round${current.round}` !== roundKey)
            return;
        ...
        const onlineActiveCount = activePlayerIds.filter((uid) => presence[uid]?.online === true).length;
        ...
        if (allDone || allOnlineDone) {
            ...
            if (!txResult.committed) {
                firebase_functions_1.logger.info('Another instance already advancing, skipping');
                return;
            }
            firebase_functions_1.logger.info(`Round ${roundKey} complete: ${doneCount}/${activePlayerIds.length}, advancing`);
            await advanceToNextRound(rtdb, roomId, current, actions, activePlayerIds, participants);
        }
    }
    catch (error) {
        firebase_functions_1.logger.error('onRoundActionSubmit error:', error);
    }
});
async function advanceToNextRound(rtdb, roomId, current, roundActions, activePlayerIds, participants) {
    const currentRound = current.round ?? 0;
    const totalRounds = current.totalRounds ?? 10;
    // 점수 집계
    const updates = {};
    const prevScores = current.scores || {};
    for (const uid of activePlayerIds) {
        const action = roundActions[uid];
        const roundScore = action?.done ? action.score || 0 : 0;
    ...
    const roundData = roundDataSnap.val();
...
//# sourceMappingURL=onRoundAction.js.map