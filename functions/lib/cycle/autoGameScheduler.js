...
exports.autoGameScheduler = void 0;
...
exports.autoGameScheduler = (0, scheduler_1.onSchedule)({
    ...
    const roomId = 'main';
    const now = Date.now();
    firebase_functions_1.logger.info('Auto game scheduler started');
    try {
        // 1) 경품 게임이 진행 중이면 스킵
        ...
        if (currentPhase !== 'IDLE' && currentPhase !== 'COOLDOWN') {
            firebase_functions_1.logger.info('Prize game in progress, skipping auto game');
        ...
        if (gamePhase && gamePhase !== 'idle' && gamePhase !== 'final_result') {
            firebase_functions_1.logger.info('A game is already running, skipping');
            return;
        }
        ...
        const prizeSnap = await db
            ...
            .get();
        if (!prizeSnap.empty) {
            firebase_functions_1.logger.info('Prize game scheduled within 10 min, skipping auto game');
            await rtdb.ref(`rooms/${roomId}/autoGame`).remove();
            return;
        }
        // 4) autoGame 현재 상태 확인
        const autoSnap = await rtdb.ref(`rooms/${roomId}/autoGame`).get();
        const autoData = autoSnap.exists() ? autoSnap.val() : null;
        if (!autoData || !autoData.phase) {
            ...
            firebase_functions_1.logger.info(`Scheduled next auto game: ${nextGame.name} at ${new Date(nextGameAt).toISOString()}`);
            return;
        }
        // 5) waiting 상태인데 nextGameAt이 지났으면 → recruit 시작
        if (autoData.phase === 'waiting') {
            const nextGameAt = autoData.nextGameAt;
            if (nextGameAt && nextGameAt <= now) {
                firebase_functions_1.logger.info('Auto game time reached, starting recruit phase');
        ...
        if (autoData.phase === 'recruiting') {
            const recruitingUntil = autoData.recruitingUntil;
            if (recruitingUntil && recruitingUntil <= now) {
                ...
                if (playerCount < 2) {
                    firebase_functions_1.logger.info(`Not enough players (${playerCount}), rescheduling`);
                    const nextGameAt = getNextHalfHour();
                    const nextGame = GAME_LIST[Math.floor(Math.random() * GAME_LIST.length)];
                    await rtdb.ref(`rooms/${roomId}/autoGame`).set({
                        phase: 'waiting',
                    ...
                    // start API 호출
                    firebase_functions_1.logger.info(`${playerCount} players joined, calling start API`);
                    ...
                    try {
                        ...
                        firebase_functions_1.logger.info('Start API response:', data);
                    }
                    catch (e) {
                        firebase_functions_1.logger.error('Failed to call start API:', e);
                        // 실패 시 리스케줄
                        const nextGameAt = getNextHalfHour();
                        const nextGame = GAME_LIST[Math.floor(Math.random() * GAME_LIST.length)];
                        await rtdb.ref(`rooms/${roomId}/autoGame`).set({
        ...
        if (autoData.phase === 'starting') {
            firebase_functions_1.logger.info('Auto game stuck in starting, resetting');
            const nextGameAt = getNextHalfHour();
            const nextGame = GAME_LIST[Math.floor(Math.random() * GAME_LIST.length)];
            await rtdb.ref(`rooms/${roomId}/autoGame`).set({
                phase: 'waiting',
        ...
        firebase_functions_1.logger.error('Auto game scheduler error:', error);
    }
});
//# sourceMappingURL=autoGameScheduler.js.map