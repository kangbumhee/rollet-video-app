...
export default function RoomClient() {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#0A0A12] text-white flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="relative z-10 w-full flex flex-col items-center">
          <div
            className="w-16 h-16 rounded-2xl bg-neon-magenta/10 border border-neon-magenta/20 flex items-center justify-center mx-auto mb-5"
            style={{ boxShadow: '0 0 20px rgba(255,45,120,0.15)' }}
          >
            <span className="text-3xl">🔒</span>
          </div>
          <h1 className="text-xl font-display mb-2">로그인이 필요합니다</h1>
          <p className="text-white/40 text-sm mb-8 text-center leading-relaxed">
            게임에 참여하려면 먼저 로그인해주세요.<br />
            Google 계정으로 간편하게 시작할 수 있어요.
          </p>
          <button
            onClick={() => router.push(`/login?redirect=/room/${roomId}`)}
            className="w-full max-w-xs flex items-center justify-center gap-3 py-3.5 px-4 bg-white text-gray-900 rounded-2xl font-medium text-sm hover:shadow-[0_0_20px_rgba(0,229,255,0.15)] active:scale-[0.98] transition-all mb-3"
          >
