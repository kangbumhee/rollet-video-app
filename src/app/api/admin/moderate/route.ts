...
async function checkAdminOrMod(token: string) {
  ...
  return { uid: decoded.uid, isAdmin: Boolean(userData.isAdmin), isModerator: Boolean(userData.isModerator) };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === 'kick') {
      ...
      return NextResponse.json({ success: true, message: `${targetDisplayName}님을 강퇴했습니다` });
    }
